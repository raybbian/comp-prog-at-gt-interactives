...
export type Route = { kind: 'play' } | { kind: 'host' } | { kind: 'briefing' };
 ...
 * `sessionStorage` and deleting the code from the address bar just falls back to it — which
 * looks exactly like the code being impossible to change.
 */
...
/**
 * The room code travels in the hash so a board can be reopened and a join link can carry it
 * — `#/host?r=482913` for the projector, `#/?r=482913` behind a QR code. It is not a secret;
 * it is on a wall in front of the room.
 */
