 ...
 *
 * `/api/*` goes to a meeting object; everything else is the built client. Keeping them on
 * one origin is not tidiness — sessions live in a cookie and in `localStorage`, both of
 * which are origin-scoped, so a second origin would mean every phone in the room losing its
 ...
 * it is already open *is* the question "is this code free", and it answers on its own single
 * thread. Two hosts hitting mint in the same second therefore cannot both be told yes, and
 * there is no lock, no registry object, and no second place for the truth to live.
 */
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    // `__` is the worker's own conversation with the object. A client reaching one would be
    // asking to open a room that is, by definition, already open.
